/*------------------------------------------------------------------------------------
	14CSE1019	PRANAV MACHINGAL
	14CSE1021	REETWIK DAS
	B-TECH 		2ND YEAR 
	CSE NIT GOA 2014-2018
------------------------------------------------------------------------------------*/
#include<iostream>
#include<stdio.h>
#include<fstream>
	...
	while(in.get(ch))
	{
		...
		{	
			...
			if(ch!=' '&&ch!='\t')
			{
				...
				{
					symbol s1;
					search.read((char *)&s1,sizeof(struct symbol));			
	...
	while(inp)	
	{
		...
		if(temp.mnemonic.format==0)
		{
			if(strcmp(temp.mnemonic.name,"BYTE")==0)
				...
				fprintf(result,"%.*X^",2*(temp1.address-temp.address),temp.obcode);
		}
